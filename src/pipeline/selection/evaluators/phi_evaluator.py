...
class PhiEvaluator(BaseEvaluator):
    def __init__(self):
        """Initialize the PhiEvaluator with a specific model."""
        self.model_name = "microsoft/Phi-3-medium-4k-instruct"
        self.model = None
        self.set_seed()
...
"""
    ...
    def score(self, data: Dict[str, Any], file_name: str, translators: list[str]) -> List[Dict[str, Any]]:
        ...
        results = []
        for sample_id, sample in tqdm(data.items(), desc=f"Scoring {file_name} with Phi"):
